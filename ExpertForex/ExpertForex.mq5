...
int OnInit() {
    ...
    current_balance = AccountInfoDouble(ACCOUNT_BALANCE); // Set initial current balance to $100
...
void OnTick() {
    ...
    if (predicted_signals[0] == 1 && positions == 0) {
        ...
        double raw_volume = NormalizeDouble(leverage * ((current_balance-5) / current_price), 2);
    ...
    if (positions != 0) {
        ...
        if (current_price <= entry_price * (1 + stop_loss_percentage)) {
            ...
            bool result = closeBuyLimits();
            if (result) {
               //Now that this is meant to compound our profits in the case of isolation, now we need to have these values actually be reflected in our account
                ...
                Print("Failed to close BuyLimit orders");
            }
...
//+------------------------------------------------------------------+
//| Function to close Buy Limit orders                               |
//+------------------------------------------------------------------+
bool closeBuyLimits() {
        ...
        if (OrderSelect(ticket)) {
            if (OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_BUY_LIMIT) {
                ...
                if (!OrderSend(request, result)) {
                    Print("Error removing Buy Limit order: ", result.retcode);
                    success = false;
                } else {
                    Print("Buy Limit order removed successfully: ", ticket);
                }
            }
        }
    }
