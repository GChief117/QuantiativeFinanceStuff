...
//+------------------------------------------------------------------+
//| Function to reset trading parameters for the new day             |
//+------------------------------------------------------------------+
void ResetDailyTrading() {
    // Reset trading parameters for the new day
    trading_day_start = TimeCurrent();
    double current_balance = AccountInfoDouble(ACCOUNT_BALANCE);

    // Use $100 from the account balance for new day trading
    initial_balance = (current_balance > 100) ? 100 : current_balance;
    daily_start_balance = initial_balance;
    //target_reached_today = false; // Reset flag for new day

    entry_price = 0;
    positions = 0;
    order_ticket = 0;

    PrintFormat("Daily trading reset at %s, new initial balance: %f", 
                TimeToString(trading_day_start), daily_start_balance);
}
...
void OnTick() {
    ...
    if (TimeToString(TimeCurrent(), TIME_SECONDS) == "00:00:00") {
        ...
        ResetDailyTrading();
        account_balance = AccountInfoDouble(ACCOUNT_BALANCE); // Update account balance after reset
        return; // Exit to avoid checking target capital right after reset
    }

    // Check if the target capital is reached and it's not exactly midnight
    if (!target_reached_today && account_balance >= target_capital) {
        PrintFormat("Target capital of %f reached at %s, stopping trading.", target_capital, TimeToString(date_time, TIME_SECONDS));
        target_reached_today = true; // Set the flag indicating target is reached today
        return;
    }
    ...
    if (!target_reached_today && predicted_signals[0] == 1 && positions == 0) {
        ...
        if (trade.Buy(volume, "GBPUSD", current_price, stop_loss_price, take_profit_price, "Buy Market Order")) {
