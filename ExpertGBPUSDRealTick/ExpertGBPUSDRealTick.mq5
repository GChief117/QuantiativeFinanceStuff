...
void OnTick() {
    ...
    static double openPrice, highPrice, lowPrice, closePrice;
    ...
    if (current_time != lastTime) {
        ...
        // Update OHLC data within the same bar
        if (current_price > highPrice) highPrice = current_price;
        if (current_price < lowPrice) lowPrice = current_price;
        closePrice = current_price;
    }

    // Print OHLC data for debugging
    PrintFormat("Time: %s, Open: %f, High: %f, Low: %f, Close: %f", TimeToString(current_time), openPrice, highPrice, lowPrice, closePrice);

    // Feature engineering
    double features[];
    ArrayResize(features, 10); // Adjust size as needed

    // Example feature calculation using OHLC data
    features[0] = (closePrice - openPrice) / openPrice * 100.0;
    features[1] = (highPrice - lowPrice) / highPrice * 100.0;
    features[2] = features[0]; // Adjust as needed for previous data
    features[3] = (features[2] > 0.01) ? 1.0 : 0.0;
    features[4] = features[3]; // Adjust as needed for future data

    // Placeholder for predicted signals
    double predicted_signals[];
    ArrayResize(predicted_signals, 1); 
    predicted_signals[0] = 1;  // Set all predicted signals to 1
    ...
    PrintFormat("Iteration at time %s, current price: %f, positions: %f, balance: %f, profit: %f, equity: %f, margin: %f, free margin: %f",
                TimeToString(current_time), current_price, positions, account_balance, profit, account_equity, account_margin, account_free_margin);

    // If target daily profit is reached, lock the previous day balance and stop trading
    if (daily_trading_balance >= target_capital) {
        PrintFormat("Target daily profit of %f reached at %s, stopping trading.", target_capital, TimeToString(current_time));
    ...
    if (!target_reached_today && predicted_signals[0] == 1 && positions == 0) {
        ...
        PrintFormat("Attempting to enter trade at %s: volume: %f, stop_loss: %f, take_profit: %f, equity: %f, margin: %f, free margin: %f",
                    TimeToString(current_time), volume, stop_loss_price, take_profit_price, account_equity, account_margin, account_free_margin);
        ...
        if (trade.Buy(volume, "GBPUSD", current_price, 0.0, 0.0, "Buy Market Order")) {
            ...
            PrintFormat("Entered position at time %s, executed price: %f, volume: %f, balance: %f, margin: %f, equity: %f, free margin: %f",
                        TimeToString(current_time), entry_price, volume, account_balance, account_margin, account_equity, account_free_margin);
        } else {
            positions = 0;
