...
int OnInit() {
    ...
    take_profit_price = 0;
    margin = 0;
    profit_cap = 200.0;  // Example profit cap, you can adjust this as needed
...
void OnTimer() {
    ...
    if (capital >= target_capital || equity - balance >= profit_cap) {
        PrintFormat("Target capital of %f or profit cap of %f reached at %s, stopping trading.", target_capital, profit_cap, TimeToString(date_time));
        return;
    }
    ...
    if (predicted_signals[0] == 1 && positions == 0) {
        ...
        double raw_volume = NormalizeDouble(leverage * ((capital) / current_price), 2);
...
void OnTick() {
    if (positions != 0) {
        ...
        if (profit_percentage <= stop_loss_percentage) {
            ...
            // Close all positions and orders to reset margin
...
void CloseAllPositionsAndOrders() {
    ...
    for (int i = PositionsTotal() - 1; i >= 0; i--) {
        if (PositionSelect(i)) {
            ...
            if (!trade.PositionClose(ticket)) {
                Print("Failed to close position: ", PositionGetString(POSITION_SYMBOL));
            }
        }
    }
    ...
    for (int j = OrdersTotal() - 1; j >= 0; j--) {
        if (OrderSelect(OrderGetTicket(j))) {
            ulong order_ticket = OrderGetInteger(ORDER_TICKET);
            if (!trade.OrderDelete(order_ticket)) {
                Print("Failed to delete order: ", OrderGetString(ORDER_SYMBOL));
            }
        }
    }

    // Ensure that margin is reset
    margin = 0;
}
