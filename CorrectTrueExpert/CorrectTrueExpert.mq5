...
// Global variables for tracking short position
bool in_short_position = false;
double short_entry_price = 0;
double profit_history[10];
int profit_index = 0;

//+------------------------------------------------------------------+
//| Custom function to convert array to string                       |
//+------------------------------------------------------------------+
string ArrayToString(double &arr[], int size) {
    string result = "[";
...
int OnInit() {
    ...
    // Initialize profit history array
    ArrayInitialize(profit_history, 0.0);
...
void OnTick() {
    ...
    if (order_ticket > 0 && PositionSelectByTicket(order_ticket)) {
        ...
        PrintFormat("Current Price: %f, Entry Price: %f, Volume: %f, Calculated Profit: %f", current_price, entry_price, volume, profit);
    } else {
        Print("No valid position selected.");
    }

    // Update profit history
    profit_history[profit_index] = profit;
    profit_index = (profit_index + 1) % 10;

    // Print profit history
    Print("Profit history: ", ArrayToString(profit_history, 10));

    // Check for significant loss
    if (!in_short_position && profit_index > 0) {
        int prev_index = (profit_index == 0) ? 9 : profit_index - 1;
        double profit_diff = profit_history[prev_index] - profit;
        PrintFormat("Previous profit: %f, Current profit: %f, Difference: %f", profit_history[prev_index], profit, profit_diff);
        if (profit_diff >= 3000) { // Adjust the threshold as needed
            if (ClosePositionByTicket(order_ticket)) {
                PrintFormat("Significant loss detected: %f. Closing long position and opening short position.", profit_diff);
                OpenShortPosition(current_price);
            } else {
                Print("Failed to close long position at significant loss");
            }
        }
    }

    // Check stop-loss for short position
    if (in_short_position && current_price >= short_entry_price * (1 - stop_loss_percentage)) {
        if (ClosePositionByTicket(order_ticket)) {
            PrintFormat("Short position stop-loss triggered at %f", current_price);
            in_short_position = false;
        } else {
            Print("Failed to close short position at stop-loss");
        }
    }
    ...
    if (!target_reached_today && predicted_signals[0] == 1 && positions == 0 && !in_short_position) {
        // Recalculate volume based on daily trading balance
        double volume = CalculateVolume(current_price);
...
//+------------------------------------------------------------------+
void OpenShortPosition(double current_price) {
    double volume = CalculateVolume(current_price);
    if (trade.Sell(volume, "GBPUSD", current_price, 0.0, 0.0, "Sell Market Order")) {
        short_entry_price = trade.ResultPrice(); // Capture the executed price
        positions = volume;
        order_ticket = trade.ResultOrder();
        in_short_position = true;
        PrintFormat("Entered short position at time %s, executed price: %f, volume: %f", TimeToString(TimeCurrent()), short_entry_price, volume);
    } else {
...
//+------------------------------------------------------------------+
double CalculateVolume(double current_price) {
